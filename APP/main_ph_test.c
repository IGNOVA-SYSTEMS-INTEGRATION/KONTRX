...
int main(void) {
    ...
    printf("\r\n=== Standalone KWS-790 pH Sensor Test ===\r\n");
    ...
    while (1) {
        ...
        if (loop_counter >= 500) {
            ...
            if (Modbus_ReadHoldingRegisters(slave_id, 0x0000, 2, registers)) {
                uint16_t raw_ph = registers[0];
                uint16_t raw_temp = registers[1];

                // Parse pH (/ 1000)
                uint16_t ph_int = raw_ph / 1000;
                uint16_t ph_frac = raw_ph % 1000;
                
                // Parse Temperature (/ 100)
                uint16_t temp_int = raw_temp / 100;
                uint16_t temp_frac = raw_temp % 100;
                
                printf("Sensor ID %02X | pH: %u.%03u | Temp: %u.%02u C\r\n", slave_id, ph_int, ph_frac, temp_int, temp_frac);
            } else {
