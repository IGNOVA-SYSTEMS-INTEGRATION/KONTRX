...
int main(void) {
    ...
    printf("\r\n=== Standalone KWS-390 EC Sensor Test ===\r\n");
    ...
    while (1) {
        ...
        if (loop_counter >= 500) {
            ...
            if (Modbus_ReadHoldingRegisters(slave_id, 0x0000, 2, registers)) {
                uint16_t raw_ec = registers[0];
                uint16_t raw_temp = registers[1];
                
                // Parse EC (1 decimal place)
                uint16_t ec_int = raw_ec / 10;
                uint16_t ec_frac = raw_ec % 10;

                // Parse Temperature (/ 100)
                uint16_t temp_int = raw_temp / 100;
                uint16_t temp_frac = raw_temp % 100;
                
                printf("Sensor ID %02X | EC: %u.%01u uS/cm | Temp: %u.%02u C\r\n", slave_id, ec_int, ec_frac, temp_int, temp_frac);
            } else {
