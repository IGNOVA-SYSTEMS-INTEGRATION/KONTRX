...
int main(void) {
    ...
    printf("\r\n=== Standalone KWS-290 Ammonia Sensor Test ===\r\n");
    ...
    while (1) {
        ...
        if (loop_counter >= 500) {
            ...
            if (Modbus_ReadHoldingRegisters(slave_id, 0x0000, 2, registers)) {
                uint16_t raw_nh3 = registers[0];
                uint16_t raw_temp = registers[1];

                // Parse NH3 (/ 10)
                uint16_t nh3_int = raw_nh3 / 10;
                uint16_t nh3_frac = raw_nh3 % 10;
                
                // Parse Temperature (/ 100)
                uint16_t temp_int = raw_temp / 100;
                uint16_t temp_frac = raw_temp % 100;
                
                printf("Sensor ID %02X | NH3: %u.%01u | Temp: %u.%02u C\r\n", slave_id, nh3_int, nh3_frac, temp_int, temp_frac);
            } else {
